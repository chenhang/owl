...
def get_teams_and_matches():
    ...
    if updated:
        ...
        write_json('winston_lab/team_hero_stats.json',
...
def get_event_player_rank():
    ...
    write_json('winston_lab/player_ranks.json', player_ranks)
    write_json('winston_lab/hero_ranks.json', hero_ranks)
...
def get_hero_stats():
    ...
    write_json('winston_lab/player_heros.json', player_heros)
    write_json('winston_lab/team_heros.json', team_heros)
...
def get_player_stats():
    ...
    for div in res.html.find('.match-div > div'):
        if 'map-wrapper' in div.attrs.get('class', []):
            ...
            category = div.text
    write_json('winston_lab/composition_stats.json', composition_stats)
    write_json('winston_lab/player_hero_stats.json', player_hero_stats)
    write_json('winston_lab/player_stats.json', player_stats)


def parse_team(team_name, team_url):
    session = HTMLSession()
    return {'id': team_url.split('id=')[-1], 'name': team_name, 'heros': [{'win_rate': div.text, 'heros': [
        span.attrs['title'].replace('soldier76', 'soldier-76') for span in div.find('span')]}
                                                                          for div in session.get(team_url).html.find(
...
def parse_match_row(row):
    ...
    team_names = [{'name': team_name_div.text,
                   'id': team_name_div.links.pop().split('id=')[-1]} for team_name_div in
                  match_res.html.find('.names-and-score', first=True).find('div')[1::2]]
    maps = []
...
# side-by-side-stats in event index page
def parse_overall_stat_div(stat_div, category="", map_name="",
                           headers=['name', 'map_played', 'kills', 'deathes', 'plusminus',
        ...
        for i, tbody in enumerate(table.find('tbody')):
            for tr in tbody.find('tr'):
                ...
                player_data += [('id', tds[0].links.pop().split('id=')[-1]), ('category', category),
                                ...
                                ('hero_kd', {div.find(
                                    'img', first=True).attrs['title'].lower().replace('.', '').replace(': ',
                                                                                                       '-'): div.text
                                             for div in tr.find('td.hero-kdu-td > div')})]
